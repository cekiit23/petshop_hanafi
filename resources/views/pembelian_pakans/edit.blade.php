...
<head>
    ...
    <title>Edit Pembelian Pakan</title>
...
<div class="container mx-auto p-4 md:p-8">
    <div class="bg-white rounded-lg shadow-lg p-6 max-w-lg mx-auto">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-3xl font-bold text-gray-800">Edit Pembelian Pakan</h2>
        ...
        <form action="{{ route('pembelian_pakans.update', $pembelianPakan->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="mb-4">
                ...
                <select name="supplier_id" class="w-full p-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500" required>
                    ...
                    @foreach($suppliers as $supplier)
                        <option value="{{ $supplier->id }}" @if($supplier->id == $pembelianPakan->supplier_id) selected @endif>{{ $supplier->nama_supplier }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="mb-4">
                ...
                <select name="pakan_id" class="w-full p-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500" required>
                    ...
                    @foreach($pakans as $pakan)
                        <option value="{{ $pakan->id }}" @if($pakan->id == $pembelianPakan->pakan_id) selected @endif>{{ $pakan->nama_pakan }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="number" name="jumlah" value="{{ $pembelianPakan->jumlah }}" class="w-full p-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500" required>
            </div>

            <div class="mb-4">
                <label for="harga_total" class="block text-gray-700 font-medium mb-2">Harga Total:</label>
                <input type="number" name="harga_total" value="{{ $pembelianPakan->harga_total }}" step="0.01" class="w-full p-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500" required>
            </div>

            <div class="mb-4">
                <label for="tanggal_pembelian" class="block text-gray-700 font-medium mb-2">Tanggal Pembelian:</label>
                <input type="date" name="tanggal_pembelian" value="{{ $pembelianPakan->tanggal_pembelian }}" class="w-full p-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500" required>
            </div>
